...
const KpiCard = ({ title, value, change, changeText, valueSuffix, isPositive, Icon, loading = false, sentiment, chartData = [] }) => {
  ...
  if (loading) return (
    <div className="w-full rounded-2xl bg-white dark:bg-dark-card border border-neutral-200 dark:border-neutral-700/60 p-5 min-h-[172px] flex flex-col justify-between animate-pulse">
      ...
      <div className="h-12 w-full bg-neutral-100 dark:bg-neutral-800 rounded-xl mt-4"/>
  ...
  return (
    <div className="group relative bg-white dark:bg-dark-card rounded-2xl border border-neutral-200/80 dark:border-neutral-700/60 p-5 overflow-hidden flex flex-col justify-between min-h-[172px] shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 cursor-default">
      ...
      <div className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none ${
        isPositive ? 'bg-green-400/[0.08]' : 'bg-red-400/[0.08]'
      } -mr-16 -mt-16`}/>
      ...
      <div className="relative z-10 flex justify-between items-start mb-2">
        ...
        {Icon && (
          <div className={`w-9 h-9 rounded-2xl flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 shadow-sm ${
            ...
            <Icon className={`w-[18px] h-[18px] ${isPositive ? 'text-green-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}/>
          </div>
        )}
      </div>

      {/* AreaChart for gradient fill effect */}
      <div className="relative z-10 w-full" style={{ height: '52px', marginTop: '4px', marginBottom: '4px' }}>
          ...
          <AreaChart data={sparkData} margin={{ top: 3, right: 0, left: 0, bottom: 3 }}>
            ...
            <Tooltip
              content={({ active, payload }) => {
                ...
                return (
                  <div className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg px-2 py-1 text-xs font-bold shadow-lg text-neutral-900 dark:text-white">
